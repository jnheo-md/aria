...
if len(sys.argv) == 1:
    ...
    if sys.argv[1] == "skip-crop":
        ...
        print(
            "usage : python aria.py [skip-crop(optional)] [manual-threshold(optional)] [filename]")
...
def draw_rectangle(x, y):
    ...
    cv2.rectangle(dragged, pt1=(ix, iy), pt2=(x, y), color=(
        255, 255, 255), thickness=-1)    # create white box
    alpha = 0.8
    mask = dragged.astype(bool)
    # merge rectangle onto image
    image[mask] = cv2.addWeighted(image, alpha, dragged, 1-alpha, 0)[mask]
...
def resizeAndCrop(src, startX, endX, startY, endY, use_openslide=False):
    ...
    if use_openslide:
        ...
        result_image = np.zeros(
            (ceil(BLOCK_SIZE * (1/width_height_ratio)), BLOCK_SIZE, 3), np.uint8)
        ...
        for iterable_x in range(ceil((slide_endX - slide_startX)/BLOCK_SIZE)):
            for iterable_y in range(ceil((slide_endY - slide_startY)/BLOCK_SIZE)):
                ...
                if (slide_startX + (iterable_x + 1) * BLOCK_SIZE) > slide_endX:
                    this_block_width = slide_endX - \
                        (slide_startX + iterable_x * BLOCK_SIZE)
                ...
                if (slide_startY + (iterable_y + 1) * BLOCK_SIZE) > slide_endY:
                    this_block_height = slide_endY - \
                        (slide_startY + iterable_y * BLOCK_SIZE)

                this_block_img = src.read_region((ceil(startX + iterable_x * BLOCK_SIZE / slide_to_total_ratio), ceil(
                    startY + iterable_y * BLOCK_SIZE / slide_to_total_ratio)), slide_level, size=(ceil(this_block_width), ceil(this_block_height)))

                # Compose destination Xs and Ys
                output_startX = ceil(
                    (iterable_x * BLOCK_SIZE)*slide_to_output_ratio)
                output_endX = output_startX + \
                    ceil(this_block_width * slide_to_output_ratio)

                output_startY = ceil(
                    (iterable_y * BLOCK_SIZE)*slide_to_output_ratio)
                output_endY = output_startY + \
                    ceil(this_block_height * slide_to_output_ratio)

                # Compose new image

                result_shape = result_image[output_startY: output_endY,
                    output_startX:output_endX].shape
                result_image[output_startY: output_endY, output_startX:output_endX] = cv2.cvtColor(cv2.resize(
                    np.array(this_block_img), dsize=(result_shape[1], result_shape[0])), cv2.COLOR_RGB2BGR)
        ...
        if (newX2 - newX1) > BLOCK_SIZE:
            ...
            result_image = cv2.resize(src, dsize=(
                BLOCK_SIZE, ceil((newY2 - newY1) * contour_resize_ratio)))
        return result_image
...
def deconvolution(src, mask, startX, endX, startY, endY, USE_OPENSLIDE):
    ...
    resizedMask = cv2.resize(mask, dsize=(
        endX - startX, endY - startY), interpolation=cv2.INTER_NEAREST)

    # create empty result image
    # opencv type, height, weight, dim
    result_image = np.zeros((endY - startY, endX - startX), np.uint8)
    ...
    for iterable_x in range(ceil((endX - startX)/BLOCK_SIZE)):
        for iterable_y in range(ceil((endY - startY)/BLOCK_SIZE)):
            ...
            if USE_OPENSLIDE:
                this_block_img = np.array(src.read_region(
                    (startX + relative_startX, startY + relative_startY), 0, size=(this_block_width, this_block_height)))
            else:
                this_block_img = src[(startY+relative_startY): (startY+relative_startY + this_block_height),
                                      (startX + relative_startX): (startX + relative_startX + this_block_width)]
                this_block_img = cv2.cvtColor(
                    this_block_img, cv2.COLOR_BGR2RGB)

            this_block_img = this_block_img[:, :, :3]

            # get mask for that region
            this_mask = cv2.cvtColor(
                resizedMask[relative_startY: relative_endY, relative_startX: relative_endX], cv2.COLOR_GRAY2BGR)

            # if mask is different than block_img than resize block 
            this_block_img = cv2.resize(this_block_img, dsize=(this_mask.shape[1], this_mask.shape[0]))
            ...
            cropped_block = img_as_ubyte(
                hed2rgb(np.stack((null, null, cropped_block[:, :, 2]), axis=-1)))
            cropped_block = 255 - \
                cv2.cvtColor(cropped_block, cv2.COLOR_RGB2GRAY)
            cropped_block = cv2.bitwise_and(
                cropped_block, cv2.cvtColor(this_mask, cv2.COLOR_BGR2GRAY))

            # save to result image
            result_image[relative_startY: relative_endY,
                relative_startX: relative_endX] = cropped_block
...
try:
    ...
    # if there is less than or equal to 2 levels, set level to last level
    small_level = slide_img.level_count-1

    small_img_size = slide_img.level_dimensions[small_level]

    small_img = np.array(slide_img.read_region(
        (0, 0), small_level, size=small_img_size))
    ...
    # finally, the image is presentable with cv2.imshow
    small_img = cv2.cvtColor(small_img, cv2.COLOR_RGB2BGR)
    # get full resolution image size
    orig_img_size = slide_img.level_dimensions[0]
    USE_OPENSLIDE = True
